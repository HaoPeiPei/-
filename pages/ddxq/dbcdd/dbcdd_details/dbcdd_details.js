...
Page({
  ...
  createPayPara() {
    ...
    if (this.data.orderId != null && this.data.orderId != "") {
        ...
        httpRequst.HttpRequst(true, '/weixin/jctnew/ashx/valet.ashx', { action: "createwxpaypara", orderId: this.data.orderId } , "POST",function(res){
            wx.hideLoading()
            if (res.Success) {
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          var memberId = app.globalData.memberId;
          var action = "cancel";
          var url = "/weixin/jctnew/ashx/valet.ashx"
          var params = {
            memberId: memberId,
            action: action,
            orderId: orderId
